...
export class VideoGenerationService {
  ...
  constructor() {
    ...
    // Output directory where Python script generates videos (in frontend/output)
    this.outputDir = path.join(rootDir, 'frontend', 'output');

    console.log('[VideoGeneration] Initialized with:');
    console.log(`  Python: ${this.pythonPath}`);
    console.log(`  Script: ${this.mainPyPath}`);
    ...
    if (!fs.existsSync(this.mainPyPath)) {
      console.error(`[ERROR] main.py not found at: ${this.mainPyPath}`);
    }
  }
  ...
  private async executeGeneration(jobId: string, topic: string, language: string): Promise<void> {
    ...
    try {
      ...
      console.log(`[${jobId}] Python: ${this.pythonPath}`);
      console.log(`[${jobId}] Script: ${this.mainPyPath}`);
      console.log(`[${jobId}] CWD: ${path.dirname(this.mainPyPath)}`);

      // Spawn Python process with stdin for interactive input
      const pythonProcess = spawn(this.pythonPath, [this.mainPyPath], {
        cwd: path.dirname(this.mainPyPath), // Run from frontend/ directory
        stdio: ['pipe', 'pipe', 'pipe'],
        env: {
          ...process.env, // Inherit environment variables (includes API keys from .env)
        }
      });
